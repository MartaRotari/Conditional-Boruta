# Core of Boruta
source("back_boruta/importance_cond.R")
source("back_boruta/tools_cond.R")

#' @export
#' @rdname Boruta
Boruta_Backward <- function(x,...)
 UseMethod("Boruta")
...
Boruta.default<-function(x,y,pValue=0.01,mcAdj=TRUE,maxRuns=100,doTrace=0,holdHistory=TRUE,getImp=getImpRfCond,...){
 ...
 assignHits<-function(hitReg,curImp){
   ...
   threshold <- ( max(curImp$shaImp)  )   # add
   curImp$imp < threshold -> hits         # add
 ...
 doTests<-function(decReg,hitReg,runs){
  ...
  ######################## ADDED ######################################
  if(runs>=1){
    toReject <- p.adjust( pbinom( hitReg-1,runs,0.5,lower.tail=FALSE) , method=pAdjMethod) < pValue
    (decReg=="Tentative" & toReject)->toReject
    
    #If attribute is significantly more frequent worse than shadowMax, its claimed Rejected (=irrelevant)
    toAccept <- p.adjust( pbinom( hitReg ,runs,0.5,lower.tail=TRUE),method=pAdjMethod) < pValue
    (decReg=="Tentative" & toAccept) -> toAccept
